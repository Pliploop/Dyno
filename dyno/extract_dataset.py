...
def extract_features(cfg: DictConfig) -> Dict[str, Any]:
    ...
    hop = cfg.get("hop", None)  # None → load_full_and_split defaults to target_n_samples (non-overlapping)
    ...
    for dataset in datasets:
        if dataset is not None:
            ...
            # Support both old LDM-style models (model.audio_encoder) and
            # new standalone AudioEncoderBase instances (model IS the encoder).
            encoder = getattr(model, "audio_encoder", model)
