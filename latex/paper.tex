...
\newcommand{\pred}{\mathcal{P}}         % velocity predictor
...
Transformer decoder reconstructs the original residual or velocity
trajectory from $\ztau$, positional decoder inputs, and an anchor
embedding $\za$.
...
temporal token describes a trajectory.
...
The default training objective combines velocity reconstruction and
KL regularisation:
%
\begin{align}
  \mathcal{L}
...
\end{align}
%
When training residual-prediction models, we replace
$\mathcal{L}_{\mathrm{vel}}$ with
%
\begin{equation}
...
% \end{equation}
%
The default model uses velocity prediction with
$\lambda_v=1$, $\lambda_r=\lambda_m=\lambda_{\mathrm{dis}}=0$,
and $\beta=0.01$.
...
$10^{-4}$, cosine decay, 5\,000 warmup steps, batch size 64, and
train for 100 epochs.  The default aggregator is a 4-layer
Transformer encoder with 8 attention heads and feed-forward
dimension 2048.  The default decoder is a RoPE-indexed Transformer
decoder with AdaLN conditioning from $\ztau$.  Unless otherwise
specified, $d=4$, $\beta=0.01$, $\Delta=1$\,s, and the start-frame
anchor $\za=z_1$ are used.
...
held-out split and are reported at the track level.  Controlled
perturbations are generated from the evaluation audio before feature
extraction, while exact permutation tests are applied directly to the
frozen embedding sequence.  Unless otherwise stated, MSPF metrics are
...
chunk shuffle, reversal, and section deletion.
...
The probing setup is intentionally simple.  We freeze all
representations and train a linear frame-level head.  For boundary
detection, the head predicts whether each frame corresponds to a
section change.  For function prediction, it predicts one of the
structural function labels.  The probe receives local frame
features, global tokens, or their concatenation:
%
\begin{align}
  u_t \in \{&z_t,\; [z_t,\zC],\; [z_t,\ztau],
    \nonumber\\
    &[z_t,\zC,\ztau],\; [\ztau,\mathrm{PE}(t)],\;
  [\zC,\mathrm{PE}(t)]\}.
  \label{eq:probe_inputs}
\end{align}
%
Including positional encoding allows global tokens to be used for
frame-level prediction.  Comparing $[z_t,\ztau]$ to $z_t$ tests
whether the temporal token provides global form information beyond
local frame features.  Comparing $\ztau$ to $\zC$ tests whether the
temporal token is more informative than order-invariant content for
...
structure analysis.  Its purpose is to test whether the frozen
representations linearly expose information about structural
boundaries and functions.  Table~\ref{tab:structure_probe} reports
the probing results for local features, global content and temporal
tokens, and their concatenations.

\begin{table}[t]
  \centering
  \setlength{\tabcolsep}{3.0pt}
  \caption{Linear probing for music structure analysis.  Boundary
  ...
  \begin{tabular}{lcccc}
    ...
    & ACC$\uparrow$ \\
    \midrule
    Local sequence $z_t$ & \res & \res & \res & \res \\
    $[z_t,\zC]$ & \res & \res & \res & \res \\
    $[z_t,\ztau]$ & \res & \res & \res & \res \\
    $[z_t,\zC,\ztau]$ & \res & \res & \res & \res \\
    \midrule
    $[\zC,\mathrm{PE}(t)]$ & \res & \res & \res & \res \\
    $[\ztau,\mathrm{PE}(t)]$ & \res & \res & \res & \res \\
    \bottomrule
  \end{tabular}
\end{table}
...
trajectory fidelity, temporal utility, and content leakage.  To make
the compression comparison independent of optimisation budget, we
also report a compute-matched sweep in which each bottleneck is
trained under both a fixed-update budget and a fixed wall-clock
budget.  The training-objective, anchor/target, masking, and multi-token
capacity ablations are reported in the appendix because they refine
the design rather than establish the core claim that a single $\ztau$
can encode track-level temporal evolution.

\begin{table*}[t]
  \centering
  \setlength{\tabcolsep}{3.0pt}
  \caption{Single-token compression sweep.  The default model uses
    $d=4$, selected to balance temporal utility and separation from the
    content token.  Fixed-update and fixed-time columns provide
  compute-matched checks for the compression trend.}
