...
The probing setup follows Toyama et al.: we freeze all representations
and use a single linear frame-level head.  Unlike their common 2\,Hz
benchmark grid, each of our probes uses the exact encoder and feature
rate used to train the evaluated checkpoint.  The local input is
therefore the checkpoint's native $z_t$ sequence, and $\zC$ and $\ztau$
are extracted from that same full-track sequence without probe-time
resampling.  Toyama et al. probe the framewise encoder sequence; our
global-token inputs are controlled extensions evaluated with the same
folds, optimisation, head, and post-processing.
For boundary
...
\end{align}
%
Full-track global tokens are repeated at every frame in the 30-second
window.  Including positional encoding allows global tokens to be used
for frame-level prediction.  Comparing $[z_t,\ztau]$ to $z_t$ tests
