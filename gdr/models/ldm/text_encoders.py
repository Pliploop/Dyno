...
class T5TextEncoder(BaseModule):
    ...
    def __init__(self, model_name = 'google-t5/t5-base', ckpt_path = None, freeze = False, max_length = 77, lora_config = None, lora_adapter_name: str = "default"):
        ...
        self.lora_enabled = self._maybe_enable_lora(
            ...
            module_name="T5 encoder",
        )

        if self.freeze and not self.lora_enabled:
            self._freeze_module_parameters(self.encoder)
...
class HuggingFaceTextEncoder(BaseModule):
    ...
    def __init__(self, model_name: str = 'bert-base-uncased', ckpt_path: str = None, freeze: bool = False, max_length: int = 512, trust_remote_code: bool = True, lora_config = None, lora_adapter_name: str = "default"):
        super().__init__(ckpt_path=ckpt_path, freeze=freeze)
        ...
        self.lora_enabled = self._maybe_enable_lora(
            ...
            adapter_name=lora_adapter_name,
            freeze_base_model=True,
            module_name="Hugging Face text encoder",
        )

        if self.freeze and not self.lora_enabled:
            self._freeze_module_parameters(self.model)
