...
class TransformerBackbone(PeftAdapterMixin, BaseModule):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.lora_enabled = self._maybe_enable_lora(
            self,
            lora_config=lora_config,
            adapter_name=lora_adapter_name,
            freeze_base_model=True,
            module_name="TransformerBackbone",
        )

        if freeze and not self.lora_enabled:
            self._freeze_module_parameters(self)
    ...
    @staticmethod
    def _prepare_joint_attention_mask(
        ...
        if attention_mask.ndim == 2:
            ...
            if attention_mask.shape[0] != batch_size:
                if attention_mask.shape[0] == 1:
                    ...
                    raise ValueError(
                        f"attention_mask batch dimension {attention_mask.shape[0]} does not match batch size {batch_size}"
                    )

            attention_mask = attention_mask != 0
            total_tokens = num_txt_tokens + num_img_tokens

            if attention_mask.shape[1] == num_txt_tokens:
                img_mask = torch.ones(
            ...
            elif attention_mask.shape[1] != total_tokens:
                raise ValueError(
                    f"attention_mask length {attention_mask.shape[1]} does not match text tokens "
                    f"({num_txt_tokens}) or total tokens ({total_tokens})"
                )
    ...
    def _forward_single(
        ...
        prepared_attention_mask = self._prepare_joint_attention_mask(
        ...
        if prepared_attention_mask is not None and "attention_mask" not in joint_attention_kwargs:
            joint_attention_kwargs["attention_mask"] = prepared_attention_mask
