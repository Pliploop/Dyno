...
class BaseModule(nn.Module):
    ...
    @staticmethod
    def _normalize_lora_config(lora_config):
        ...
        try:
            ...
            raise ImportError(
                "PEFT is required when `lora_config` is provided. Please install it with `pip install peft`."
            ) from e

        if isinstance(lora_config, PeftConfig):
            return lora_config

        if isinstance(lora_config, dict):
            config_dict = dict(lora_config)
            config_dict.pop("_target_", None)
            config_dict.pop("_convert_", None)
            return LoraConfig(**config_dict)

        raise TypeError(
            f"`lora_config` must be a dict or a PEFT config instance, got {type(lora_config).__name__}."
        )

    def _maybe_enable_lora(self, module, lora_config=None, adapter_name="default", freeze_base_model=True, module_name="module"):
        if lora_config is None:
            return False

        if not hasattr(module, "add_adapter"):
            raise TypeError(f"{module_name} does not support PEFT adapters via `add_adapter`.")

        if freeze_base_model:
            self._freeze_module_parameters(module)

        normalized_lora_config = self._normalize_lora_config(lora_config)
        module.add_adapter(normalized_lora_config, adapter_name=adapter_name)
        return True
